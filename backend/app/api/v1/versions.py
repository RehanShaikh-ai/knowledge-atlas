...
def get_note_version(
    ...
    return NoteVersionResponse.model_validate(v)
...
def get_note_diff(
    note_id: uuid.UUID,
    db: Annotated[Session, Depends(get_db)],
    from_commit: Annotated[str, Query(alias="from", min_length=40, max_length=40)],
    to_commit: Annotated[str, Query(alias="to", min_length=40, max_length=40)],
) -> DiffResponse:
...
def restore_note_version(
    ...
    restored_content = git_service.read_note(note.workspace_id, note_id)
    ...
    version = version_service.create_version(
        ...
        note_id=note_id,
        author_id=request.author_id,
        message=f"Restore version from {request.commit_hash[:7]}",
        is_ai_edit=False,
    )
