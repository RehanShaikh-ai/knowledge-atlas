...
export const DiffViewer: React.FC<DiffViewerProps> = ({ noteId, fromHash, toHash, className = '' }) => {
  ...
  if (isLoading) {
    return (
      <div className={`flex flex-col items-center justify-center p-12 text-overlay1 bg-white/[0.02] border border-white/[0.08] backdrop-blur-xl rounded-2xl ${className}`}>
  ...
  if (error) {
    return (
      <div className={`p-6 bg-red-500/10 border border-red-500/20 text-red-300 rounded-2xl flex items-start gap-4 ${className}`}>
  ...
  if (!diff || diff.trim() === '') {
    return (
      <div className={`p-12 text-center text-overlay1 bg-white/[0.02] border border-white/[0.08] backdrop-blur-xl rounded-2xl flex flex-col items-center ${className}`}>
  ...
  return (
    <div className={`flex flex-col bg-white/[0.02] border border-white/[0.08] backdrop-blur-xl rounded-2xl overflow-hidden ${className}`}>
      ...
      <div className="overflow-x-auto py-2 h-[400px] overflow-y-auto bg-black/40 scrollbar-thin scrollbar-thumb-surface1">
        {renderDiff()}
      </div>
    </div>
  );
