...
export const KnowledgeExplorer: React.FC<KnowledgeExplorerProps> = ({
  ...
  if (isLoading) {
    return (
      <aside
        ...
        className={cn(
          'w-96 bg-slate-950/80 border border-white/[0.08] rounded-2xl p-5 backdrop-blur-2xl shadow-2xl shadow-black/60 flex flex-col items-center justify-center min-h-[400px]',
          className
        )}
      >
  ...
  if (error || !entity) {
    return (
      <aside
        data-testid="knowledge-explorer"
        className={cn(
          'w-96 bg-slate-950/80 border border-white/[0.08] rounded-2xl p-5 backdrop-blur-2xl shadow-2xl shadow-black/60 flex flex-col',
          className
        )}
      >
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-4 text-center">
          ...
          <button
            ...
            className="mt-3 text-xs bg-white/[0.06] hover:bg-white/[0.1] border border-white/[0.08] px-3 py-1.5 rounded-lg text-slate-300 transition-colors"
          >
            Retry
          </button>
        </div>
  ...
  return (
    <aside
      data-testid="knowledge-explorer"
      className={cn(
        'w-96 bg-slate-950/80 border border-white/[0.08] rounded-2xl p-5 backdrop-blur-2xl shadow-2xl shadow-black/60 flex flex-col text-slate-200 z-30',
        className
      )}
    >
      {/* ── Header ── */}
      <div className="flex items-start justify-between pb-3 border-b border-white/[0.06]">
        <div className="min-w-0 pr-2">
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            ...
            {entity.is_manual ? (
              <span className="text-[10px] font-medium text-amber-400 bg-amber-500/10 border border-amber-500/25 px-2 py-0.5 rounded-full flex items-center gap-1">
                <User size={10} /> Manual
              </span>
            ) : (
              <span className="text-[10px] font-medium text-sky-400 bg-sky-500/10 border border-sky-500/25 px-2 py-0.5 rounded-full flex items-center gap-1">
        ...
        <button
          onClick={onClose}
          className="text-slate-400 hover:text-slate-200 p-1.5 rounded-lg hover:bg-white/[0.06] transition-colors shrink-0"
      ...
      {/* ── Action Toolbar ── */}
      <div className="flex items-center gap-1.5 py-2.5 border-b border-white/[0.06]">
        {onEditEntity && (
          <button
            onClick={() => onEditEntity(entity)}
            className="flex-1 text-xs font-medium bg-white/[0.04] hover:bg-white/[0.08] text-slate-200 border border-white/[0.08] hover:border-white/[0.16] px-2.5 py-1.5 rounded-xl transition-all flex items-center justify-center gap-1.5"
        ...
        {onAddRelationship && (
          <button
            ...
            className="flex-1 text-xs font-medium bg-sky-500/10 hover:bg-sky-500/20 text-sky-300 border border-sky-500/25 px-2.5 py-1.5 rounded-xl transition-all flex items-center justify-center gap-1.5"
        ...
        <button
          ...
          disabled={isDeleting}
          className="text-xs font-medium bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 border border-rose-500/25 px-2.5 py-1.5 rounded-xl transition-all flex items-center justify-center gap-1.5 disabled:opacity-50"
      ...
      {/* ── Tab Switcher ── */}
      <div className="flex items-center border-b border-white/[0.06] text-xs font-medium pt-2">
        <button
              ...
              <>
                ...
                {outgoingLinks.map(({ edge, targetNode }) => (
                  <div
                    ...
                    className="p-3 bg-white/[0.03] hover:bg-white/[0.05] border border-white/[0.06] hover:border-white/[0.12] rounded-xl space-y-1.5 transition-all"
                  >
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="font-mono text-sky-300 bg-sky-500/10 px-1.5 py-0.5 rounded border border-sky-500/25">
                    ...
                    <div className="flex items-center justify-between pt-1">
                      ...
                      {targetNode && onSelectEntity && (
                        <button
                          ...
                          className="text-[10px] text-sky-400 hover:text-sky-300 flex items-center gap-1 ml-2 shrink-0 transition-colors"
                        >
                ...
                {incomingLinks.map(({ edge, sourceNode }) => (
                  ...
                  >
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="font-mono text-indigo-300 bg-indigo-500/10 px-1.5 py-0.5 rounded border border-indigo-500/25">
                        ← {edge.relationship_type}
                      </span>
                      <span className="text-[10px] font-mono text-slate-400">
                        {Math.round(edge.confidence * 100)}% conf
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {onNavigateToNote && (
                      <button
                        ...
                        className="text-[10px] text-amber-400 hover:text-amber-300 flex items-center gap-0.5 shrink-0 transition-colors"
                      >
